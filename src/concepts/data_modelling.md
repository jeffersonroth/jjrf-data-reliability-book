...
The primary data sources are then evaluated, ETL tools are used to fetch data from several sources and load it into a staging area of the relational database server.
...
Kimball dimensional modelling allows users to construct several star schemas to fulfill various reporting needs.
...
When the data is denormalized, all the data available is pulled (as advocated by Inmon) while using a denormalized design (as advocated by Kimball). One example is the [Carry Forward](https://resilientbiz.com/the-resilient-hybrid-methodology-data-warehouse/) method.
